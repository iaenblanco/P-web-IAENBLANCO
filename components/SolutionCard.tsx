  ...
  return (
    ...
    >
      <div className="mb-4" aria-hidden="true">
        <IconComponent
          className={`h-12 w-12 ${iconColor} transition-transform duration-300 group-hover:rotate-12`}
          aria-label={iconAlt}
        />
      </div>
      <h3 id={`solution-title-${title}`} className="text-2xl font-bold mb-2">{title}</h3>
      <p id={`solution-desc-${title}`} className="text-gray-300 mb-4">{description}</p>

      <div className="mb-6">
        <h4 className="text-sm font-semibold text-cyan-400 mb-2">Ideal para:</h4>
        <p className="text-sm text-gray-400">{targetAudience}</p>
      </div>

      <div className="mb-6">
        <h4 className="text-sm font-semibold text-cyan-400 mb-2">Beneficios principales:</h4>
